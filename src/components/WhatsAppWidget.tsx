...
export default function WhatsAppWidget() {
  ...
  return (
    <>
      {/* Chat Window */}
      {isOpen && (
        <div className="fixed bottom-24 right-6 z-50 w-96 max-w-[calc(100vw-3rem)] animate-slideUp">
      ...
      <button
        ...
        className={`fixed bottom-6 right-6 z-50 bg-white text-green-500 rounded-full px-4 py-3 shadow-lg hover:shadow-xl transition-all duration-300 flex items-center gap-2 ${
          isOpen ? 'scale-0' : 'scale-100 hover:scale-105'
        }`}
        aria-label="Abrir chat de WhatsApp"
      >
        <MessageCircle className="h-6 w-6" />
        <span className="font-medium text-gray-700">Conversemos</span>
      </button>
      ...
      {isOpen && (
        <button
          onClick={() => setIsOpen(false)}
          className="fixed bottom-6 right-6 z-50 bg-white text-gray-600 rounded-full p-4 shadow-lg hover:shadow-xl transition-all duration-300 animate-fadeIn hover:bg-gray-50"
          aria-label="Cerrar chat"
        >
          <X className="h-6 w-6" />
        </button>
